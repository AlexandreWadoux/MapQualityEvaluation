gg_sun <- function(mods, obs, colorval = NULL, colorval.name = NULL, x.axis_begin, x.axis_end, y.axis_end, by, label){
  
  # compute normalized ME, Eq. 9 Jolliff et al., (2009) - Y-axis
  model_nME <- plyr::laply(mods, .fun = function(x, y){(mean(y) - mean(x))/sd(y)}, y = obs)
  
  model_sigmaD <- 1 # AW: sign is alwasy positive (change from target diagram)
  
  # compute the normalized SDE, called (model_uRMSDnorm below) SDE* in Eq. 8 Jolliff et al., (2009) - X-axis second part
  ...
  # if not color variable on the points are specified, put the correlation
  ...
  ### AW: the are determined based on the correlation, to be changed, see Jollief et al., (2009), Eq. 14
  ...
  x.lab_frame <- data.frame(lab = range(x.axis_begin, x.axis_end),zero = 0)
  ...
  base.plot <- ggplot(data = data,  aes(x = nME, y = uRMSDnorm_sigmaD)) +
    geom_polygon(data = circ_pol, aes(x, y, fill = lab)) + 
    scale_fill_manual(values = c('r>0' = '#FFFA85','r>0.5' = '#FFE269', 'r>0.9' = '#FFCB69', 'r>0.95'=  '#E65C6F'),
                      name = '', 
    ...
    theme(axis.line.x=element_blank(),
          ...
          axis.ticks.x = element_blank(), axis.ticks.length.x = unit(-0.3, "cm"),
